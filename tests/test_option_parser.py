...
from rynner.option_parser import *
...
class TestOptionParser(unittest.TestCase):
    ...
    def instantiate(self, opt_map=None):
        ...
        self.defaults = MM()
        self.option_parser = OptionParser(self.opt_map, self.submit_cmd,
                                          self.defaults)
    ...
    def assert_parse(self, opt_map, input, output):
        ...
        context = self.option_parser.parse(input)
    ...
    def test_option_parser_can_call_run(self):
        self.instantiate()
        self.option_parser.run(
            self.mock_conn, {
                'options': ['Some Option Result', 'Another Option Result'],
                'script': 'command'
            }, '/some/remote/path')

    def test_option_parser_single_string_opt_map_parsed(self):
        opt_map = [
            ('#FAKE --memory={}', 'memory'),
        ]
    ...
    def test_option_parser_multiple_string_opt_map_parsed(self):
        opt_map = [
            ('#FAKE --memory={}', 'memory'),
            ('#FAKE --cpus={}', 'cpus'),
        ]
    ...
    def test_option_parser_duplicate_string_matches(self):
        opt_map = [
            ('#FAKE --memory={}', 'memory'),
    ...
    def test_option_parser_compound_string_matches(self):
        opt_map = [
    ...
    def test_input_dict_untouched(self):
        ...
        self.instantiate(opt_map)
        self.option_parser.parse(input)
    ...
    def test_run_method_calls_connection(self):
        ...
        self.option_parser.run(self.mock_conn, context, '/some/remote/path')
